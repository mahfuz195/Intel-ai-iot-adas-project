...
class Detector(object):
    def __init__(self, ie, model_path, weights_path, threshold=0.3, device='MYRIAD'):
        ...
        print ('USING: ', device , ' Input layer Name: ', self._input_layer_name)
        
        self.is_async = True 
        if(self.is_async):
            self.feed_dict = {}
            self.input_blob = self._input_layer_name
        ...
        self._ie = ie
        if self.is_async:
            self._exec_model = self._ie.load_network(network=model, num_requests = 2, device_name = device)
        else:
